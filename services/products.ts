export async function getProducts({ page, pageSize, orderby, name }: { page?: number, pageSize?: number, orderby?: string, name?: string }) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/products?${params.toString()}`, { next: { revalidate: 0 } })
  return res.json()
}

export async function getProduct(slug: string | number) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/products/${slug}`, { next: { revalidate: 0 } })
  return res.json()
}